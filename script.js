...
const apiUrl = "https://603e722548171b0017b2f7a1.mockapi.io/people/";
...
function agregarUsuarios (usuario) {
  ...
  <li data-id="${usuario.id}">
    <h3 role="button">${usuario.fullName}</h3>
    <button>X</button>
  </li>
`;
}
...
function camposEnviados(event){
  ...
  ponerUsuario(datosForm)
}
function ponerUsuario(datosForm) {
  fetch(apiUrl, {
    method: "POST",
...
enviarForm.addEventListener("submit", camposEnviados);